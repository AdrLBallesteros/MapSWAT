...
class BaseDialog_GEE(QDialog, Ui_BaseDialog_GEE):
    ...
    def Clip_New(self):
        ...
        #Connect to editor's closing function
    ...
    def GEE_Mask(self):
        ...
        try:
            Map.addLayer(self.polygon, {'color': 'black'}, 'Mask_GEE')
        except: 
            pass
...
#         self.canvas.refresh()
    ...
    def Clip_Extension(self):
        ...
        #Add GEE layer to Map
        try:
            Map.addLayer(bufferedPoint, {'color': 'black'}, 'Mask_GEE')
        except: 
            pass
    ...
    def Clip_Autobasin(self):
        ...
        try:
            Map.addLayer(selectedBasin, {'color': 'black'}, 'Mask_GEE')
        except: 
            pass
                ...
                while i < 100:
                    ...
                    try:
                        ...
                        if resolution > resolution_ini:
                            QMessageBox.information(None, "Map Resolution Adjustment", f"GEE request exceeds the limit of 5 MB. Raster resolution adjusted to {resolution} meters.")
                        break
...
# #           #For geometry
#             try:               
#                 #Download the clipped area directly using attributes
#                 url = LULC.getDownloadURL({'name': 'LANDUSE',
#                                            'scale': resolution, 
#                                            'crs': crs_target_id, 
#                                            'region': self.polygon.getInfo(),  
#                                            'fileFormat':  'GeoTIFF'})
#                  
#                 #QMessageBox.information(None, "LULC URL", url)
#                 
#             #For feature collection
#             except:                
#                 #Download the clipped area directly using attributes
...
#                                            'fileFormat':  'GeoTIFF'})
            ...
            ##Render the raster showed in the CANVAS
#             band = 1
#             value_counts = Counter(LULC_sorted_values)
#             unique_value_count = len(value_counts)
#             # Create the color ramp
#             fnc = QgsColorBrewerColorRamp.create({'colors': str(unique_value_count), 'schemeName': 'Spectral'})
#             # Get colors for each unique value
#             colors = [fnc.color(i / float(unique_value_count - 1)) for i in range(unique_value_count)]
#             # Create the classes for the raster renderer
#             classes = [QgsPalettedRasterRenderer.Class(value, colors[i], str(value)) for i, value in enumerate(value_counts.keys())]
#             # Set the renderer for the raster layer
#             renderer = QgsPalettedRasterRenderer(LULC.dataProvider(), band, classes)
#             LULC.setRenderer(renderer)
#             LULC.triggerRepaint()
        ...
        if self.checkBox_Soil.isChecked():
            ...
            ##Render the raster showed in the CANVAS
#             band = 1
#             value_counts = Counter(SOIL_sorted_values)
#             unique_value_count = len(value_counts)
#             # Create the color ramp
#             fnc = QgsColorBrewerColorRamp.create({'colors': str(unique_value_count), 'schemeName': 'Spectral'})
#             # Get colors for each unique value
...
#             classes = [QgsPalettedRasterRenderer.Class(value, colors[i], str(value)) for i, value in enumerate(value_counts.keys())]
#             # Set the renderer for the raster layer
#             renderer = QgsPalettedRasterRenderer(SOIL.dataProvider(), band, classes)
#             SOIL.setRenderer(renderer)
#             SOIL.triggerRepaint()
