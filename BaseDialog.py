...
class BaseDialog(QDialog, Ui_BaseDialog):
    ...
    def __init__(self, iface):
        ...
        self.mQgsFileWidget_DEM.setFilter("TIFF files (*.tif);;ERDAS IMAGINE files (*.img)")
        self.mQgsFileWidget_Landuse.setFilter("TIFF files (*.tif);;ERDAS IMAGINE files (*.img)")
        self.mQgsFileWidget_Soil.setFilter("TIFF files (*.tif);;ERDAS IMAGINE files (*.img)")
    ...
    def AddMaps(self): 
        ...
        if self.checkBox_Soil.isChecked():
            ...
            if  crs_Soil == QgsCoordinateReferenceSystem('EPSG:4326'):
                ...
                try:   
                    ...
                    QMessageBox.warning(None, "ADD LAYERS", "Please, select a raster layer in SOIL.")
        ...
        if self.checkBox_Landuse.isChecked():
            ...
            if  crs_Landuse == QgsCoordinateReferenceSystem('EPSG:4326'):
                ...
                try:
                    ...
                    QMessageBox.warning(None, "ADD LAYERS", "Please, select a raster layer in LANDUSE.")
        ...
        if self.checkBox_DEM.isChecked():
            ...
            if  crs_DEM == QgsCoordinateReferenceSystem('EPSG:4326'):
                ...
                try: 
                    ...
                    QMessageBox.warning(None, "ADD LAYERS", "Please, select a raster layer in DEM.")
        ...
        if   self.lineX.isModified() and self.lineY.isModified():
            ...
            writer = QgsVectorFileWriter(FolderPath + r"/MapSWAT/WGS84/OUTLET.shp",
                             "UTF-8",
                             QgsFields(),
                             QgsWkbTypes.Point, 
                             crs_Outlet, 
    ...
    def merge(self):
        ...
        selectedFiles = QtWidgets.QFileDialog.getOpenFileNames(self,"Select files to merge","","TIFF files (*.tif)")
        filenames = selectedFiles[0]
        
        self.progressBar.setValue(50)
        
        #Process for merging layers
        params = {'INPUT': filenames,
                  'PCT':False,
                  'SEPARATE':False,
                  'NODATA_INPUT':None,
                  'NODATA_OUTPUT':None,
                  ...
                  'EXTRA':'',
                  'DATA_TYPE':5,
                  'OUTPUT':FolderPath + "\MapSWAT\SWAT_INPUT_MAPS\INFO_GIS\MERGE\DEM_merge.tif"}

        processing.run("gdal:merge", params)
        
        if os.path.isfile(FolderPath + "\MapSWAT\SWAT_INPUT_MAPS\INFO_GIS\MERGE\DEM_merge.tif"):
            #Copy path in FileWidget_DEM
            self.mQgsFileWidget_DEM.setFilePath(FolderPath + "\MapSWAT\SWAT_INPUT_MAPS\INFO_GIS\MERGE\DEM_merge.tif")
    ...
    def Clip_New(self):
        ...
        #Conectar a fucion de cierre de editor
    ...
    def Clip_Extension(self):
        ...
        params = {'INPUT':FolderPath + '/MapSWAT/WGS84/OUTLET.shp',
                  ...
                  'MITER_LIMIT':2,
                  'DISSOLVE':False,
                  'OUTPUT':FolderPath + '/MapSWAT/SWAT_INPUT_MAPS/INFO_GIS/POLYGON/POLYGON_buffer.shp'}

        processing.run("native:buffer", params)
        
        #Reproject buffer to WGS84
        params2 = {'INPUT':FolderPath + '/MapSWAT/SWAT_INPUT_MAPS/INFO_GIS/POLYGON/POLYGON_buffer.shp',
                  'TARGET_CRS':QgsCoordinateReferenceSystem('EPSG:4326'),
                  'OUTPUT':FolderPath + '/MapSWAT/WGS84/POLYGON.shp'}
        processing.run("native:reprojectlayer",params2) 
   
        #Add WGS84 buffer to QGIS
        layer = QgsVectorLayer(FolderPath + '/MapSWAT/WGS84/POLYGON.shp', 'Buffer POLYGON', "ogr")
        QgsProject.instance().addMapLayer(layer)        
    ...
    def CREATE_INPUTS(self): 
        ...
        if self.checkBox_Soil.isChecked():
            ...
            if  crs_Soil == QgsCoordinateReferenceSystem('EPSG:4326'): 
                ...
                #Clip the SOIL using the Polygon in WGS84             
                params = {'INPUT':FolderPath + '/MapSWAT/WGS84/SOIL.tif',
                          'MASK':FolderPath + '/MapSWAT/WGS84/POLYGON.shp',
                          'NODATA':None,
                          'ALPHA_BAND':False,
                          'CROP_TO_CUTLINE':True,
            ...
            crs_target = self.mQgsProjection_Target.crs()                       
            params2 = {'INPUT':FolderPath + '/MapSWAT/WGS84/CLIPPED/SOIL.tif',
            ...
            params = {'INPUT_RASTER':FolderPath + '/MapSWAT/WGS84/CLIPPED/SOIL.tif',
                      'RASTER_BAND':1,
            ...
            layer = QgsVectorLayer(FolderPath + '/MapSWAT/WGS84/SOIL_Values.shp', 'SOIL_Values', "ogr")
            ...
            with open(FolderPath + "\MapSWAT\SWAT_INPUT_MAPS\SOIL\Soil_lookup.csv", 'w', newline='') as csvfile:
        ...
        if self.checkBox_Landuse.isChecked():
            ...
            if  crs_Landuse == QgsCoordinateReferenceSystem('EPSG:4326'):
                ...
                #Clip the LANDUSE using the Polygon in WGS84            
                params = {'INPUT':FolderPath + '/MapSWAT/WGS84/LANDUSE.tif',
                          'MASK':FolderPath + '/MapSWAT/WGS84/POLYGON.shp',
                          'NODATA':None,
                          'ALPHA_BAND':False,
                          'CROP_TO_CUTLINE':True,
            ...
            crs_target = self.mQgsProjection_Target.crs()                       
            params2 = {'INPUT':FolderPath + '/MapSWAT/WGS84/CLIPPED/LANDUSE.tif',
                      'SOURCE_CRS':QgsCoordinateReferenceSystem('EPSG:4326'),
                      'TARGET_CRS':crs_target,
                      'RESAMPLING':0,
                      'NODATA':None,
            ...
            #Obtain raster classes in the area
            params = {'INPUT_RASTER':FolderPath + '/MapSWAT/WGS84/CLIPPED/LANDUSE.tif',
                      'RASTER_BAND':1,
                      'INPUT_VECTOR':FolderPath + '/MapSWAT/WGS84/POLYGON.shp',
                      'COLUMN_PREFIX':'',
            ...
            #Remove excess columns
            layer = QgsVectorLayer(FolderPath + '/MapSWAT/WGS84/LANDUSE_Values.shp', 'LANDUSE_Values', "ogr")
            ...
            with open(FolderPath + "\MapSWAT\SWAT_INPUT_MAPS\LANDUSE\Landuse_lookup.csv", 'w', newline='') as csvfile:
                filewriter = csv.writer(csvfile)
        ...
        if self.checkBox_DEM.isChecked():
            ...
            if  crs_DEM == QgsCoordinateReferenceSystem('EPSG:4326'):
                ...
                #Clipping the DEM using the Polygon in WGS84            
                params = {'INPUT':FolderPath + '/MapSWAT/WGS84/DEM.tif',
                          'MASK':FolderPath + '/MapSWAT/WGS84/POLYGON.shp',
                          'NODATA':None,
                          'ALPHA_BAND':False,
                          'CROP_TO_CUTLINE':True,
            ...
            crs_target = self.mQgsProjection_Target.crs()                       
            params2 = {'INPUT':FolderPath + '/MapSWAT/WGS84/CLIPPED/DEM.tif',
                      'SOURCE_CRS':QgsCoordinateReferenceSystem('EPSG:4326'),
                      'TARGET_CRS':crs_target,
                      'RESAMPLING':0,
                      'NODATA':None,
        ...
        #QMessageBox.information(None, "SWAT maps", "Process completed.")
